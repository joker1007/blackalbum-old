...
class MovieFactory extends events.EventEmitter
  ...
  get_md5_hash: ->
    ...
    rs = fs.createReadStream @filename, {start: 0, end: 300 * 1024}
    rs.on 'data', (d) ->
      md5sum.update d
  ...
  merge_thumbnail: (count) ->
    ...
    exec "convert +append #{files} \"thumbs/#{@movie.title}-#{@movie.md5_hash}.jpg\"", (err3, stdout, stderr) =>
      try
        throw "[Failed] Merge Thumbnails: #{@filename}" if err3
